...
@Component
public class WxMpImageHandler extends AbstractWxMpHandler {
...
//		response = buildOpenInfoResponse(wxMessage);
	...
	private void redirectMessage2Kf(WxMpXmlMessage wxMessage,
		...
		String content = user.getNickName() + "[" + user.getId() + "] 发的图片:";
		...
		WxMpCustomMessage customImageMessage = WxMpCustomMessage.IMAGE().toUser(appProperty.defaultKf).mediaId(wxMessage.getMediaId()).build();
