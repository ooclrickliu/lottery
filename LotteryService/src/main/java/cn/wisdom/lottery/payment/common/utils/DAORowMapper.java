		...
		for (int columnIndex=0;columnIndex++!=columnCount;){
			  	...
			  	case Types.NUMERIC: //处理整数和浮点数等数字类型
					if (this.direct) {
						...
						if (wapping.getScale(columnIndex) <= 0){//没有小数位
							...
							paramClassName = "java.lang.Integer,int,java.lang.Boolean,boolean,java.lang.Float,float,java.lang.Double,double";
						} else{//有小数位所以为浮点类型
							value = rs.getFloat(columnIndex);
			...
			if (this.direct){ //直接映射情况下，对剩余字段类型，比如 VARCHAR 类型或 CHAR 类型，进行补充处理
				...
				for (Method method : methods) {
					...
					if (DAORowMapper.classNameMap.get(params[0].getName()) == DAORowMapper.ClassType.STRING && value == null){
						value = "";
					}
					//执行 setter
					try {
